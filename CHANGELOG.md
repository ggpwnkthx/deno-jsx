...
- Corrected `createJSXVNode` JSDoc to accurately describe it creates vnodes for elements, fragments, or components (not just "element vnodes")

### Added

- JSDoc documentation for all export groups in mod.ts re-exports

## [0.1.7] - 2026-03-27

### Added

- Comprehensive JSDoc documentation improvements across all source files
- Added JSDoc to `Fragment` interface in types.ts
- Added JSDoc to `JSX` namespace and all member types in jsx-runtime.ts and jsx-dev-runtime.ts
- Enhanced existing JSDocs with `@remarks`, `@example`, and detailed descriptions for all VNode types, `Normalizable`, `JsxDevSource`, `JsxIntrinsicElementProps`, `JsxIntrinsicElements`, and all JSX types
